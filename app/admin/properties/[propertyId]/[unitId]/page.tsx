...
export default function PropertyUnitPage() {
  ...
  const showNotification = (message,typ= 'success') => {
  ...
  const handleDelete = async () => {
   ...
   const handleEditSubmit = async (formData) => {
    try {
      const response = await axios.patch(`http://127.0.0.1:5556/units/${unitId}`, formData);
      ...
      <div className="bg-white text-black rounded-lg md:rounded-xl shadow-md p-4 md:p-6 border border-gray-200">
        ...
        <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-4 pb-4 border-b border-gray-200 gap-3">
          ...
          <div className="flex flex-wrap gap-2">
            {current_lease ? (
              <>
                <button className="btn btn-outline btn-primary" onClick={() => setShowPaymentModal(true)}>Record Payment</button>
                <button className="btn btn-outline btn-error" onClick={() => setShowEndLeaseModal(true)}>End Lease</button>
              </>
            ) : (
              <button className="btn btn-outline border-blue-600 text-blue-600" onClick={() => setShowAssignModal(true)}>Assign Tenant</button>
            )}
          </div>
        </div>
                ...
                <tbody>
                  {payment_history.map((payment) => (
                    <tr key={payment.payment_id} className="border-b">
                      <td className="py-2 px-4">{new Date(payment.period_start).toLocaleDateString()} - {new Date(payment.period_end).toLocaleDateString()}</td>
        ...
        {unit.payment_status && (
          <div className="mt-6 bg-gray-50 p-4 rounded border">
            ...
            <div className="grid grid-cols-2 gap-2 text-sm">
              ...
              <p><strong>Expected:</strong> ${unit.payment_status.expected_total_rent}</p>
              <p><strong>Paid:</strong> ${unit.payment_status.total_paid}</p>
              <p><strong>Balance:</strong> ${unit.payment_status.balance_due}</p>
