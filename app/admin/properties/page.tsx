...
export default function PropertiesPage() {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-white to-blue-50 flex items-center justify-center p-4 sm:p-6">
        <div className="text-center">
          ...
          <p className="mt-3 sm:mt-4 text-sm sm:text-base">Loading properties...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-white to-blue-50">
      <div className="space-y-6 sm:space-y-8 sm:space-y-10 pt-4 sm:pt-6 px-4 sm:px-6 pb-6 sm:pb-8">
        {/* Header */}
        <div className="text-center space-y-2 sm:space-y-3">
          ...
          <p className="text-sm sm:text-base md:text-lg text-gray-500 font-medium max-w-md mx-auto leading-relaxed">
        ...
        {properties.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 md:gap-8 max-w-7xl mx-auto">
            {properties.map((property) => (
              <Link key={property.id} href={`/admin/properties/${property.id}`}>
                <div className="group bg-white rounded-2xl sm:rounded-3xl shadow-sm hover:shadow-xl transition-all duration-500 p-4 sm:p-6 cursor-pointer border border-gray-100 hover:border-blue-100 relative overflow-hidden">
                  {/* Gradient accent */}
                  ...
                  <div className="space-y-3 sm:space-y-4">
                    <h2 className="text-lg sm:text-xl md:text-2xl font-bold text-gray-800 group-hover:text-blue-600 transition-colors duration-300 leading-tight line-clamp-2">
                  ...
                  {/* View details hint */}
                  <div className="absolute bottom-3 sm:bottom-4 md:bottom-6 right-3 sm:right-4 md:right-6 opacity-0 group-hover:opacity-100 transform translate-y-1 sm:translate-y-2 group-hover:translate-y-0 transition-all duration-300">
                    <div className="p-1.5 sm:p-2 bg-blue-500 text-white rounded-full">
          ...
          <div className="max-w-2xl mx-auto bg-white rounded-2xl sm:rounded-3xl shadow-sm p-6 sm:p-8 md:p-12 lg:p-16 text-center border border-gray-100">
            <div className="w-16 h-16 sm:w-20 sm:h-20 md:w-24 md:h-24 mx-auto mb-4 sm:mb-6 bg-gradient-to-br from-blue-50 to-purple-50 rounded-2xl sm:rounded-3xl flex items-center justify-center">
              <HomeModernIcon className="h-8 w-8 sm:h-10 sm:w-10 md:h-12 md:w-12 text-gray-400" />
            </div>
            <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-gray-800 mb-2 sm:mb-3 md:mb-4">No Properties Yet</h2>
            <p className="text-gray-500 text-sm sm:text-base md:text-lg mb-4 sm:mb-6 md:mb-8 max-w-md mx-auto leading-relaxed">
              Start building your rental portfolio by adding your first property
            </p>
            <button 
              onClick={() => setShowForm(true)}
              className="group bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 text-white py-2.5 sm:py-3 md:py-4 px-6 sm:px-8 md:px-10 lg:px-12 rounded-xl sm:rounded-2xl font-semibold text-sm sm:text-base md:text-lg shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 inline-flex items-center gap-2 sm:gap-3 w-full sm:w-auto justify-center max-w-sm mx-auto"
            >
              <PlusIcon className="h-4 w-4 sm:h-5 sm:w-5" />
              Add Your First Property
              <ArrowRightIcon className="h-3 w-3 sm:h-4 sm:w-4 group-hover:translate-x-0.5 sm:group-hover:translate-x-1 transition-transform duration-300" />
            </button>
          </div>
        )}
      </div>
