...
export default function AddUnitPopup({ propertyId, onClose }) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const payload = {
        ...
        property_id: parseInt(propertyId), // ✅ Ensure it's an integer
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-40 flex justify-center items-center z-50">
      <div className="bg-white p-6 rounded-lg w-full max-w-xl space-y-4 text-black">
        ...
        <input name="monthly_rent" type="number" className="input input-bordered w-full" placeholder="Monthly Rent" onChange={handleChange} />
        <input name="deposit_amount" type="number" className="input input-bordered w-full" placeholder="Deposit Amount" onChange={handleChange} />
